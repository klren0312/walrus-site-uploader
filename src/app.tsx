...
import { ConnectButton, useCurrentAccount, useCurrentWallet, useSignAndExecuteTransaction, useSignTransaction } from '@mysten/dapp-kit';
import { Transaction } from '@mysten/sui/transactions';
import { fromB64, toB64,toHEX,bcs } from '@mysten/bcs';
...
function idToBase36(id: Uint8Array) {
  ...
  const string = encoding.slice(skip)
      .map(c => BASE36[c])
      .join('');
...
function get_blobId(blobId: string){ 
  ...
  let number = U256.deserialize( new Deserializer(Base64.toUint8Array(blobId)));
...
console.log("get_blobId ",get_blobId('2YLU3Usb-WoJAgoNSZUNAFnmyo8cfV8hJYt2YdHL2Hs'))

const id = new Uint8Array(Buffer.from ("0xaf4eafdac7039b742b5ee59d702428df36670b94164c3c388b74c5dca0f0e41e".replace("0x",""),"hex"));   
console.log(idToBase36(id));   
console.log( Buffer.from ("c100d0cb55401738f9783890e7679d97a6aab5856a67c009c6f72e711de4c669","hex"))
export function App() {
        ...
        let site = txb.moveCall({
         ...
         for (const v of data){
          let html = txb.moveCall({
            ...
            arguments:[txb.pure.string(`/${v.name}`), txb.pure.string(getMimeType(v.name)), txb.pure.string("plaintext"), txb.pure.u256(get_blobId(v.blobId))]
          });
          txb.moveCall({
            package: "0x514cf7ce2df33b9e2ca69e75bc9645ef38aca67b6f2852992a34e35e9f907f58",
            module: "site",
